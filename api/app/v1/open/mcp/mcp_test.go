...
func TestMCPReadScopeDeniedDraftTool(t *testing.T) {
	...
	if drafts, err := app.DefaultStore().ListDrafts(fixture.superID, fixture.projectID, fixture.serviceID); err != nil || len(drafts) != 0 {
...
func TestMCPJSONRPCToolsListIncludesV01Tools(t *testing.T) {
	...
	for _, tool := range result.Tools {
		if tool.Name == "publish_api_schema" || tool.Name == "publish_api_version" {
			t.Fatalf("tools/list exposed v0.2 publish tool %q", tool.Name)
		}
...
func TestMCPJSONRPCToolsCallExecutesV01Tools(t *testing.T) {
	...
	token, err := app.DefaultStore().CreateMCPToken(fixture.writerID, "writer", []int{app.ScopeAPIRead, app.ScopeAPIDraft}, nil)
	if err != nil {
	...
	assertRPCResult(t, callMCPToolRPC(t, fixture.router, token.Token, "list_services", gin.H{"project_id": fixture.projectID}), "list_services")
	assertRPCResult(t, callMCPToolRPC(t, fixture.router, token.Token, "list_api_versions", gin.H{"project_id": fixture.projectID, "service_id": fixture.serviceID}), "list_api_versions")
	latestSchema := assertRPCResult(t, callMCPToolRPC(t, fixture.router, token.Token, "get_latest_schema", gin.H{"project_id": fixture.projectID, "service_id": fixture.serviceID, "branch_id": fixture.branchID}), "get_latest_schema")
	...
	endpointDetail := assertRPCResult(t, callMCPToolRPC(t, fixture.router, token.Token, "get_endpoint_detail", gin.H{"project_id": fixture.projectID, "service_id": fixture.serviceID, "version_id": versionOne.ID, "endpoint_id": endpoint.ID}), "get_endpoint_detail")
	...
	compareResult := assertRPCResult(t, callMCPToolRPC(t, fixture.router, token.Token, "compare_api_versions", gin.H{"project_id": fixture.projectID, "service_id": fixture.serviceID, "from_version_id": versionOne.ID, "to_version_id": versionTwo.ID}), "compare_api_versions")
	var diff struct {
		ID      string `json:"id"`
		Summary struct {
	...
	changeSummary := assertRPCResult(t, callMCPToolRPC(t, fixture.router, token.Token, "get_change_summary", gin.H{"project_id": fixture.projectID, "service_id": fixture.serviceID, "diff_id": diff.ID}), "get_change_summary")
	...
	createResult := assertRPCResult(t, callMCPToolRPC(t, fixture.router, token.Token, "create_api_version_draft", gin.H{"project_id": fixture.projectID, "service_id": fixture.serviceID, "branch_id": fixture.branchID, "version_name": "1.2.0", "schema_content": mcpTestOpenAPI("draftCreate")}), "create_api_version_draft")
	var draft struct {
		ID string `json:"id"`
	}
	...
	assertRPCResult(t, callMCPToolRPC(t, fixture.router, token.Token, "get_api_version_draft", gin.H{"project_id": fixture.projectID, "service_id": fixture.serviceID, "draft_id": draft.ID}), "get_api_version_draft")
	assertRPCResult(t, callMCPToolRPC(t, fixture.router, token.Token, "update_api_version_draft", gin.H{"project_id": fixture.projectID, "service_id": fixture.serviceID, "draft_id": draft.ID, "branch_id": fixture.branchID, "version_name": "1.2.0", "schema_content": mcpTestOpenAPIWithReport("draftUpdated")}), "update_api_version_draft")
	assertRPCResult(t, callMCPToolRPC(t, fixture.router, token.Token, "submit_api_version_draft", gin.H{"project_id": fixture.projectID, "service_id": fixture.serviceID, "draft_id": draft.ID}), "submit_api_version_draft")
}
...
func TestMCPJSONRPCErrorsAreStructured(t *testing.T) {
	...
	assertRPCError(t, callMCPToolRPC(t, fixture.router, token.Token, "publish_api_version", gin.H{}), -32602, "invalid tool")
	assertRPCError(t, callMCPToolRPC(t, fixture.router, token.Token, "list_services", gin.H{}), -32602, "invalid arguments")
	assertRPCError(t, callMCPToolRPC(t, fixture.router, token.Token, "create_api_version_draft", gin.H{"project_id": fixture.projectID, "service_id": fixture.serviceID, "branch_id": fixture.branchID, "version_name": "blocked", "schema_content": mcpTestOpenAPI("blocked")}), -32003, "permission denied")
	...
	assertRPCError(t, callMCPToolRPC(t, fixture.router, token.Token, "get_endpoint_detail", gin.H{"project_id": fixture.projectID, "service_id": fixture.serviceID, "version_id": "missing", "endpoint_id": "missing"}), -32004, "not found")
}
...
func TestMCPJSONRPCEvidenceWriter(t *testing.T) {
	...
	endpointDetail := callMCPRPCBody(t, fixture.router, token.Token, gin.H{"jsonrpc": "2.0", "id": "task-11-get-endpoint-detail", "method": "tools/call", "params": gin.H{"name": "get_endpoint_detail", "arguments": gin.H{"project_id": fixture.projectID, "service_id": fixture.serviceID, "version_id": version.ID, "endpoint_id": endpoint.ID}}})
...
func newMCPFixture(t *testing.T) mcpFixture {
	...
	service, err := store.CreateService(superUser.ID, project.ID, "mcp-service", "MCP Service", "", "/mcp")
	if err != nil {
	...
	return mcpFixture{router: router, superID: superUser.ID, readerID: readerUser.ID, writerID: writerUser.ID, projectID: project.ID, serviceID: service.ID, branchID: branchID}
}
...
func publishMCPFixtureVersion(t *testing.T, fixture mcpFixture, versionName, schema string) *app.ContractVersion {
	t.Helper()
	store := app.DefaultStore()
	draft, err := store.CreateDraft(fixture.superID, fixture.projectID, fixture.serviceID, app.DraftInput{BranchID: fixture.branchID, VersionName: versionName, SchemaContent: schema})
	if err != nil {
		t.Fatalf("CreateDraft(%s) error = %v", versionName, err)
	}
	if _, err := store.SubmitDraft(fixture.superID, fixture.projectID, fixture.serviceID, draft.ID); err != nil {
		t.Fatalf("SubmitDraft(%s) error = %v", versionName, err)
	}
	published, err := store.ReviewDraft(fixture.superID, fixture.projectID, fixture.serviceID, draft.ID, "approve")
	if err != nil {
...
func firstMCPEndpoint(t *testing.T, fixture mcpFixture, versionID string) *app.Endpoint {
	t.Helper()
	endpoints, err := app.DefaultStore().ListEndpoints(fixture.superID, fixture.projectID, fixture.serviceID, versionID, "")
	if err != nil {
