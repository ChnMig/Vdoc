...
| MCP Token | 给 AI 工具通过 MCP 查询或发布接口契约的带权限 token。 |

## MVP 使用流程

1. Admin 创建 Team。
2. Admin 创建 Project。
3. Admin 邀请成员并分配项目级角色。
4. Admin 或后端开发创建 Service。
5. 后端开发或 AI 上传 OpenAPI 3.x 文档。
6. Vdoc 校验并保存 Raw Schema。
7. Vdoc 创建不可变 Contract Version。
...
- 项目级角色：`Reader`、`Writer`、`Admin`
- 通过 Web API 上传 OpenAPI 3.x，后续接入 MCP
- 每个 Service 下的不可变接口契约版本
- 接口列表和接口详情查询
- 版本比较和语义 Diff
- Breaking-change 摘要
- 先做只读 MCP tools，再做写入 tools
...
Storage
  ...
  - 对象存储保存 Raw / Normalized OpenAPI 快照
